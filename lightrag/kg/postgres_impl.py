...
class PostgreSQLDB:
    ...
    async def _migrate_doc_status_drop_soft_delete(self):
        ...
        try:
            ...
            logger.error(
                f"MIGRATION FAILED (drop soft-delete from LIGHTRAG_DOC_STATUS): {e}. "
    ...
    async def check_tables(self):
        ...
        # Migrate doc status to drop soft-delete fields if they exist
        try:
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        if is_namespace(self.namespace, NameSpace.KV_STORE_TEXT_CHUNKS):
            ...
            for i, (k, v) in enumerate(data.items(), start=1):
                ...
                # Merge user-provided metadata with org_id into meta column (Scogo)
...
class PGDocStatusStorage(DocStatusStorage):
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        # upserts that omit it (or pass '' / NULL) will NOT clear it.
        sql = """insert into LIGHTRAG_DOC_STATUS(workspace,id,org_id,content_summary,content_length,chunks_count,status,file_path,chunks_list,track_id,metadata,error_msg,content_hash,created_at,updated_at,token_usage)
        ...
        #   error_msg, content_hash, created_at, updated_at, token_usage)
        batch: list[tuple] = []
        skipped: list[str] = []
...
SQL_TEMPLATES = {
    ...
    # Pipeline-derived columns guarded with COALESCE so partial upserts don't overwrite.
    # Scogo meta column preserves user metadata + org_id.
